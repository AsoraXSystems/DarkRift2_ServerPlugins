...
inline uint dtHashRef(dtPolyRef a)
{
    ...
    {
        public static uint dtHashRef(UInt32 a)
        {
            a += ~(a << 15);
            a ^= (a >> 10);
            a += (a << 3);
    ...
    public partial class Detour
    {
        ...
        public const UInt16 DT_NULL_IDX = UInt16.MaxValue; //(dtNodeIndex)~0;
        ...
        public class dtNode
        {
            ...
            public UInt32 id;				    //< Polygon ref the node corresponds to.
            ///
        ...
        public class dtNodePool
        {
            ...
            private UInt16[] m_first;
            private UInt16[] m_next;
...

            {
                ...
                m_next = new UInt16[m_maxNodes];
                m_first = new UInt16[hashSize];
            ...
            public int getMemUsed()
            {
                return
                    ...
                    sizeof(UInt16) * m_maxNodes +
                    sizeof(UInt16) * m_hashSize;
            }
            ...
            public UInt16 getFirst(int bucket)
            {
                return m_first[bucket];
            }
            public UInt16 getNext(int i)
            {
                return m_next[i];
            }

            public dtNode findNode(UInt32 id)
            {
                uint bucket = (uint)(dtHashRef(id) & (m_hashSize - 1));
                UInt16 i = m_first[bucket];
                while (i != DT_NULL_IDX)
                {
                    if (m_nodes[i].id == id)
                        return m_nodes[i];
            ...
            public dtNode getNode(UInt32 id)
            {
                uint bucket = (uint)(dtHashRef(id) & (m_hashSize - 1));
                UInt16 i = m_first[bucket];
                dtNode node = null;
