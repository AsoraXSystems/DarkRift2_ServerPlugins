...
namespace RecastDetour.Detour
{
    ...
    */
    ...
    public class NavMesh
    {
        ...
        private Detour.dtNavMeshParams m_params;			//< Current initialization params. TODO: do not store this info twice.
        ...
        //dtMeshTile**
        private Detour.dtMeshTile[] m_posLookup;			//< Tile hash lookup.
        private Detour.dtMeshTile m_nextFree;				//< Freelist of tiles.
        private Detour.dtMeshTile[] m_tiles;				//< List of tiles.
        ...
        public NavMesh()
        {
	        ...
	        m_polyBits = 0;
#endif
            m_params = new Detour.dtNavMeshParams();
            m_orig[0] = 0;
            m_orig[1] = 0;
            m_orig[2] = 0;
        }
        ...
        public dtStatus init(Detour.dtNavMeshParams navMeshParams)
        {
            //memcpy(&m_params, params, sizeof(dtNavMeshParams));
            m_params = navMeshParams.Clone();
            Detour.dtVcopy(m_orig, navMeshParams.orig);
            m_tileWidth = navMeshParams.tileWidth;
            m_tileHeight = navMeshParams.tileHeight;

            // Init tiles
            m_maxTiles = navMeshParams.maxTiles;
            m_tileLutSize = (int)Detour.dtNextPow2((uint)(navMeshParams.maxTiles / 4));
            if (m_tileLutSize == 0)
                m_tileLutSize = 1;
            m_tileLutMask = m_tileLutSize - 1;

            //m_tiles = (dtMeshTile*)dtAlloc(sizeof(dtMeshTile)*m_maxTiles, DT_ALLOC_PERM);
            m_tiles = new Detour.dtMeshTile[m_maxTiles];
            Detour.dtcsArrayItemsCreate(m_tiles);

            if (m_tiles == null)
                return (Detour.DT_FAILURE | Detour.DT_OUT_OF_MEMORY);
            //m_posLookup = (dtMeshTile**)dtAlloc(sizeof(dtMeshTile*)*m_tileLutSize, DT_ALLOC_PERM);
            m_posLookup = new Detour.dtMeshTile[m_tileLutSize];
            Detour.dtcsArrayItemsCreate(m_posLookup);
            if (m_posLookup == null)
                return Detour.DT_FAILURE | Detour.DT_OUT_OF_MEMORY;
...
#if DT_POLYREF64
#else
            m_tileBits = (dtStatus)Detour.dtIlog2(Detour.dtNextPow2((uint)navMeshParams.maxTiles));
            m_polyBits = (dtStatus)Detour.dtIlog2(Detour.dtNextPow2((uint)navMeshParams.maxPolys));
        ...
        public dtStatus init(Detour.dtRawTileData rawTile, int flags)
        {
            //C#: Using an intermediate class dtRawTileData because Cpp uses a binary buffer.

            // Make sure the data is in right format.
            //dtMeshHeader header = (dtMeshHeader*)data;
            Detour.dtMeshHeader header = rawTile.header;
            ...
            Detour.dtNavMeshParams navMeshParams = new Detour.dtNavMeshParams();
            Detour.dtVcopy(navMeshParams.orig, header.bmin);
        ...
        public Detour.dtNavMeshParams getParams()
        {
            return m_params;
        }
        ...
        public int findConnectingPolys(float[] va, int vaStart, float[] vb, int vbStart,
            Detour.dtMeshTile tile, int side,
            dtPolyRef[] con, float[] conarea, int maxcon)
        {
            ...
            Detour.calcSlabEndPoints(va, vaStart, vb, vbStart, amin, amax, side);
            float apos = Detour.getSlabCoord(va, vaStart, side);

            // Remove links pointing to 'side' and compact the links array. 
            float[] bmin = new float[2];
            float[] bmax = new float[2];
            ushort m = (ushort)(Detour.DT_EXT_LINK | (ushort)side);
            int n = 0;
            ...
            for (int i = 0; i < tile.header.polyCount; ++i)
            {
                ...
                for (int j = 0; j < nv; ++j)
                {
                    ...
                    float bpos = Detour.getSlabCoord(tile.verts, vcStart, side);
                    ...
                    Detour.calcSlabEndPoints(tile.verts, vcStart, tile.verts, vdStart, bmin, bmax, side);

                    if (!Detour.overlapSlabs(amin, amax, bmin, bmax, 0.01f, tile.header.walkableClimb))
                        continue;
        ...
        public void unconnectExtLinks(Detour.dtMeshTile tile, Detour.dtMeshTile target)
        {
        ...
        public void connectExtLinks(Detour.dtMeshTile tile, Detour.dtMeshTile target, int side)
        {
            if (tile == null)
                return;
            ...
            for (int i = 0; i < tile.header.polyCount; ++i)
            {
                ...
                for (int j = 0; j < nv; ++j)
                {
                    ...
                    int nnei = findConnectingPolys(tile.verts, vaStart, tile.verts, vbStart, target, Detour.dtOppositeTile(dir), nei, neia, 4);
                    for (int k = 0; k < nnei; ++k)
                    {
                        uint idx = Detour.allocLink(tile);
                        if (idx != Detour.DT_NULL_LINK)
                        {
                            Detour.dtLink link = tile.links[idx];
                            link.polyRef = nei[k];
                            link.edge = (byte)j;
                            link.side = (byte)dir;
                            ...
                            if (dir == 0 || dir == 4)
                            {
                                ...
                                if (tmin > tmax)
                                    Detour.dtSwap(ref tmin, ref tmax);
                                link.bmin = (byte)(Detour.dtClamp(tmin, 0.0f, 1.0f) * 255.0f);
                                link.bmax = (byte)(Detour.dtClamp(tmax, 0.0f, 1.0f) * 255.0f);
                            }
                            else if (dir == 2 || dir == 6)
                            {
        ...
        /// Builds external polygon links for a tile.
        public void connectExtOffMeshLinks(Detour.dtMeshTile tile, Detour.dtMeshTile target, int side)
        {
            if (tile == null)
                return;
            ...
            byte oppositeSide = (side == -1) ? (byte)0xff : (byte)Detour.dtOppositeTile(side);

            for (int i = 0; i < target.header.offMeshConCount; ++i)
            {
                Detour.dtOffMeshConnection targetCon = target.offMeshCons[i];
                if (targetCon.side != oppositeSide)
                    continue;

                Detour.dtPoly targetPoly = target.polys[targetCon.poly];
                ...
                if (Detour.dtSqr(nearestPt[0] - targetCon.pos[pIndex]) + Detour.dtSqr(nearestPt[2] - targetCon.pos[pIndex + 2]) > Detour.dtSqr(targetCon.rad))
                    continue;
                // Make sure the location is on current mesh.
                //float* v = &target.verts[targetPoly.verts[1]*3];
                int vIndex = targetPoly.verts[1] * 3;
                Detour.dtVcopy(target.verts, vIndex, nearestPt, 0);

                // Link off-mesh connection to target poly.
                uint idx = Detour.allocLink(target);
                if (idx != Detour.DT_NULL_LINK)
                {
                    Detour.dtLink link = target.links[idx];
                ...
                {
                    uint tidx = Detour.allocLink(tile);
                    if (tidx != Detour.DT_NULL_LINK)
                    {
                        ushort landPolyIdx = (ushort)decodePolyIdPoly(polyRef);
                        Detour.dtPoly landPoly = tile.polys[landPolyIdx];
                        Detour.dtLink link = tile.links[tidx];
        ...
        public void connectIntLinks(Detour.dtMeshTile tile)
        {
            if (tile == null)
                return;
            ...
            for (int i = 0; i < tile.header.polyCount; ++i)
            {
                ...
                for (int j = poly.vertCount - 1; j >= 0; --j)
                {
                    ...
                    if (poly.neis[j] == 0 || (poly.neis[j] & Detour.DT_EXT_LINK) != 0)
                        continue;
        ...
        /// Builds internal polygons links for a tile.
        public void baseOffMeshLinks(Detour.dtMeshTile tile)
        {
            if (tile == null)
                return;
            ...
            {
                Detour.dtOffMeshConnection con = tile.offMeshCons[i];
                Detour.dtPoly poly = tile.polys[con.poly];
                ...
                if (polyRef == 0)
                    continue;
                // findNearestPoly may return too optimistic results, further check to make sure. 
                if (Detour.dtSqr(nearestPt[0] - con.pos[0]) + Detour.dtSqr(nearestPt[2] - con.pos[2]) > Detour.dtSqr(con.rad))
                    continue;
                // Make sure the location is on current mesh.
                //float* v = &tile.verts[poly.verts[0]*3];
                int vIndex = poly.verts[0] * 3;
                Detour.dtVcopy(tile.verts, vIndex, nearestPt, 0);
        ...
        public void closestPointOnPoly(dtPolyRef polyRef, float[] pos, int posStart, float[] closest, ref bool posOverPoly)
        {
            ...
            if (poly.getType() == (byte)Detour.dtPolyTypes.DT_POLYTYPE_OFFMESH_CONNECTION)
            {
                ...
                float d0 = Detour.dtVdist(pos, posStart, tile.verts, v0Start);
                float d1 = Detour.dtVdist(pos, posStart, tile.verts, v1Start);
                float u = d0 / (d0 + d1);
            ...
            Detour.dtVcopy(closest, 0, pos, posStart);
            if (!Detour.dtDistancePtPolyEdgesSqr(pos, posStart, verts, nv, edged, edget))
            {
            ...
            for (int j = 0; j < pd.triCount; ++j)
            {
                ...
                float h = .0f; ;
                if (Detour.dtClosestHeightPointTriangle(pos, posStart, vArrays[0], vIndices[0], vArrays[1], vIndices[1], vArrays[2], vIndices[2], ref h))
                {
                    closest[1] = h;
                    break;
                }
        ...
        public dtPolyRef findNearestPolyInTile(Detour.dtMeshTile tile,
            ...
            float[] nearestPt)
        {
            ...
            Detour.dtVsub(bmin, 0, center, centerStart, extents, 0);
            Detour.dtVadd(bmax, 0, center, centerStart, extents, 0);
            ...
            for (int i = 0; i < polyCount; ++i)
            {
                ...
                Detour.dtVsub(diff, 0, center, centerStart, closestPtPoly, 0);
                if (posOverPoly)
                {
            ...
            dtPolyRef[] polys, int maxPolys)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < tile.header.polyCount; ++i)
                {
                    ...
                    Detour.dtVcopy(bmin, 0, tile.verts, vIndex);
                    Detour.dtVcopy(bmax, 0, tile.verts, vIndex);
                    for (int j = 1; j < p.vertCount; ++j)
                    {
                        //v = &tile.verts[p.verts[j]*3];
                        vIndex = p.verts[j] * 3;
                        Detour.dtVmin(bmin, 0, tile.verts, vIndex);
                        Detour.dtVmax(bmax, 0, tile.verts, vIndex);
                    }
        ...
        /// @return The status flags for the operation.
        public dtStatus addTile(Detour.dtRawTileData rawTileData, int flags, dtTileRef lastRef, ref dtTileRef result)
        {
            //C#: Using an intermediate class dtRawTileData because Cpp uses a binary buffer.

            // Make sure the data is in right format.
            Detour.dtMeshHeader header = rawTileData.header;
            if (header.magic != Detour.DT_NAVMESH_MAGIC)
                return Detour.DT_FAILURE | Detour.DT_WRONG_MAGIC;
            if (header.version != Detour.DT_NAVMESH_VERSION)
                return Detour.DT_FAILURE | Detour.DT_WRONG_VERSION;
            ...
            else
            {
                ...
                Detour.dtMeshTile target = m_tiles[tileIndex];
                Detour.dtMeshTile prev = null;
                tile = m_nextFree;
            ...
            for (int i = 0; i < 8; ++i)
            {
                ...
                for (int j = 0; j < nneis; ++j)
                {
                    ...
                    connectExtLinks(neis[j], tile, Detour.dtOppositeTile(i));
                    connectExtOffMeshLinks(tile, neis[j], i);
                    connectExtOffMeshLinks(neis[j], tile, Detour.dtOppositeTile(i));
                }
            }
        ...
        public Detour.dtMeshTile getTileAt(int x, int y, int layer)
        {
            // Find tile based on hash.
            int h = Detour.computeTileHash(x, y, m_tileLutMask);
            Detour.dtMeshTile tile = m_posLookup[h];
            while (tile != null)
            {
                if (tile.header != null &&
                    tile.header.x == x &&
        ...
        public int getNeighbourTilesAt(int x, int y, int side, Detour.dtMeshTile[] tiles, int maxTiles)
        {
            int nx = x, ny = y;
            switch (side)
            {
        ...
        public int getTilesAt(int x, int y, Detour.dtMeshTile[] tiles, int maxTiles)
        {
            int n = 0;
        ...
        public Detour.dtMeshTile getTileByRef(dtTileRef tileRef)
        {
            if (tileRef != 0)
                return null;
            ...
            if ((int)tileIndex >= m_maxTiles)
                return null;
            Detour.dtMeshTile tile = m_tiles[tileIndex];
        ...
        public Detour.dtMeshTile getTile(int i)
        {
            return m_tiles[i];
        }
        ...
        /// @return The status flags for the operation.
        public dtStatus getTileAndPolyByRef(dtPolyRef polyRef, ref Detour.dtMeshTile tile, ref Detour.dtPoly poly)
        {
            if (polyRef == 0)
                return Detour.DT_FAILURE;
        ...
        public dtStatus getTileAndPolyByRef(dtPolyRef polyRef, ref Detour.dtMeshTile tile, ref Detour.dtPoly poly, ref uint ip)
        {
            if (polyRef == 0)
                return Detour.DT_FAILURE;
            uint salt = 0, it = 0;
        ...
        ///  @param[out]	tile	The tile containing the polygon.
        ///  @param[out]	poly	The polygon.
        public void getTileAndPolyByRefUnsafe(dtPolyRef polyRef, ref Detour.dtMeshTile tile, ref Detour.dtPoly poly)
        {
            uint salt = 0, it = 0, ip = 0;
            decodePolyId(polyRef, ref salt, ref it, ref ip);
            tile = m_tiles[it];
        ...
        public void getTileAndPolyByRefUnsafe(dtPolyRef polyRef, ref Detour.dtMeshTile tile, ref Detour.dtPoly poly, ref uint ip)
        {
            uint salt = 0, it = 0;
            decodePolyId(polyRef, ref salt, ref it, ref ip);
            tile = m_tiles[it];
        ...
        /// @return The status flags for the operation.
        public dtStatus removeTile(dtTileRef tileRef, out Detour.dtRawTileData rawTileData)
        {
            rawTileData = null;
            ...
            Detour.dtMeshTile prev = null;
            Detour.dtMeshTile cur = m_posLookup[h];
            while (cur != null)
            {
                if (cur == tile)
                {
            ...
            // Reset tile.
            if ((tile.flags & (int)Detour.dtTileFlags.DT_TILE_FREE_DATA) != 0)
            {
                // Owns data
                //dtFree(tile.data);
                tile.data = null;
        ...
        public dtTileRef getTileRef(Detour.dtMeshTile tile)
        {
        ...
        public dtPolyRef getPolyRefBase(Detour.dtMeshTile tile)
        {
            if (tile == null) return 0;
        ...
        public dtStatus getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, float[] startPos, float[] endPos)
        {
            ...
            // Get current polygon
            decodePolyId(polyRef, ref salt, ref it, ref ip);
            if (it >= (uint)m_maxTiles) return Detour.DT_FAILURE | Detour.DT_INVALID_PARAM;
            if (m_tiles[it].salt != salt || m_tiles[it].header == null) return Detour.DT_FAILURE | Detour.DT_INVALID_PARAM;
            Detour.dtMeshTile tile = m_tiles[it];
            if (ip >= (uint)tile.header.polyCount) return Detour.DT_FAILURE | Detour.DT_INVALID_PARAM;
            Detour.dtPoly poly = tile.polys[ip];
            ...
            Detour.dtVcopy(startPos, 0, tile.verts, poly.verts[idx0] * 3);
            Detour.dtVcopy(endPos, 0, tile.verts, poly.verts[idx1] * 3);
        ...
        public Detour.dtOffMeshConnection getOffMeshConnectionByRef(dtPolyRef polyRef)
        {
            uint salt = 0, it = 0, ip = 0;
        ...
        {
            if (polyRef == 0) return Detour.DT_FAILURE;
            uint salt = 0, it = 0, ip = 0;
            decodePolyId(polyRef, ref salt, ref it, ref ip);
            if (it >= (uint)m_maxTiles) return Detour.DT_FAILURE | Detour.DT_INVALID_PARAM;
            if (m_tiles[it].salt != salt || m_tiles[it].header == null) return Detour.DT_FAILURE | Detour.DT_INVALID_PARAM;
